...
import { Trophy, Coins, Award, Crown, Sparkles, RefreshCw } from 'lucide-react';
    ...
    <div className="leaderboard-container animate-fade-in">
      <div className="leaderboard-header glass-card">
        <div className="header-title-box">
          ...
          <div>
            <h2>🏆 명예의 전당</h2>
            <p>최고의 단위 변환 마스터 도전자 TOP 10 순위입니다!</p>
          </div>
        </div>

        <button className="btn-refresh" onClick={fetchRankings} disabled={loading}>
          <RefreshCw size={18} className={loading ? 'animate-spin' : ''} />
          {loading ? '새로고침 중...' : '순위 갱신'}
        </button>
      </div>
            ...
            <div key={entry.uid || index} className={`rank-card rank-${rank <= 3 ? rank : 'normal'}`}>
              <div className="rank-left">
                <div className="rank-number">
                  {rank === 1 && <Crown size={24} className="crown-gold" />}
                  {rank === 2 && <Crown size={22} className="crown-silver" />}
                  {rank === 3 && <Crown size={22} className="crown-bronze" />}
                  <span>{rank}위</span>
                </div>

                <div className="user-details">
                  <span className="user-name">{entry.displayName}</span>
                  <span className={`provider-chip ${entry.authProvider}`}>
                    {entry.authProvider === 'google' ? 'Google' : '간편'}
                  </span>
                </div>
              </div>
              ...
              <div className="rank-right">
                {tab === 'gold' ? (
                  <div className="score-chip text-yellow">
                    <Coins size={18} />
                    <span>{entry.gold.toLocaleString()} G</span>
                  </div>
                ) : (
